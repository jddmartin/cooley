...
class Potential():
    ...
    def __init__(self, potential_function,
        ...
        # potential function, and user_length is the unit for the argument of
    ...
    def v(self, r):
        ...
        if self.j == 0:
            ...
            return (val + self.hbar2_div_2m * self.j * (self.j + 1.0) / r**2)
...
def integrate(Potential, energy, rstart, rstep, max_points,
    ...
    Arguments:
      Potential: an object with a member function:   ".v(r)"
                 ...
                 (The class "Potential" in this  module is a convenient way
                  to make this object from a potential function).
    ...
    for i in range(2, max_points):
        ...
        yprev = (1.0 - h2 / 12.0 *
                 (Potential.v(rprev) - energy) / hbar2_div_2m) * psi[-2]
        ycurrent = (1.0 - h2 / 12.0 *
                    (Potential.v(rcurrent) - energy) /
                    hbar2_div_2m) * psi[-1]
        ynew = (2.0 * ycurrent - yprev +
                h2 * (Potential.v(rcurrent) - energy) / hbar2_div_2m * psi[-1])
        psi.append(ynew / (1.0 - h2 / 12.0 * (Potential.v(rnext) - energy) /
                           hbar2_div_2m))
        ...
        if rstep < 0.0:  # inwards integration stopping criteria *not met*:
            ...
            raise InwardsError(
                "Inwards integration stopping criteria not met.")
    return psi
...
def update_energy(Potential, energy, rmin, rmax, npoints):
    ...
    yinner = psi[m_index - 1] * \
        (1 - h2 / 12.0 * (Potential.v(rinner) - energy) / hbar2_div_2m)
    ym = psi[m_index] * \
        (1 - h2 / 12.0 * (Potential.v(rm) - energy) / hbar2_div_2m)
    youter = psi[m_index + 1] * \
        (1 - h2 / 12.0 * (Potential.v(router) - energy) / hbar2_div_2m)
...
def find_single_eigen(Potential,
    ...
    Arguments:
      Potential: an object with a member function:   ".v(r)"
                 ...
                 a convenient way to make this object from
